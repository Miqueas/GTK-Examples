...
static GtkWidget* build_stack(void);

const static gchar* APP_ID = "io.github.Miqueas.GTK-Examples.C.Gtk3.Stack2";
...
static void on_app_startup(GApplication* self, gpointer data) {
  GtkWidget* window, *stack, *sidebar, *box;
...
GtkWidget* build_stack(void) {
  ...
  for (gint i = 0; i < 3; i++) {
    ...
    gtk_stack_add_titled(GTK_STACK(stack), label_widget, name, title);
  }
