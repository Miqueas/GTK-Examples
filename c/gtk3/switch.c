...
const static gchar* APP_ID = "io.github.Miqueas.GTK-Examples.C.Gtk3.Switch";
const static gchar* APP_TITLE = "GtkSwitch";
...
static void on_app_startup(const GApplication* self, gpointer data) {
  ...
  gtk_container_add(GTK_CONTAINER(window), switch_button);
  gtk_window_set_title(GTK_WINDOW(window), APP_TITLE);
  gtk_window_set_default_size(GTK_WINDOW(window), 400, 400);

  gtk_widget_set_valign(switch_button, GTK_ALIGN_CENTER);
  gtk_widget_set_halign(switch_button, GTK_ALIGN_CENTER);
