...
const static gchar* APP_ID = "io.github.Miqueas.GTK-Examples.C.Gtk3.ToggleButton";
const static gchar* APP_TITLE = "GtkToggleButton";
...
static void on_app_startup(const GApplication* self, gpointer data) {
  ...
  GtkWidget* toggle_button = gtk_toggle_button_new_with_label("Toggle Me");

  gtk_container_add(GTK_CONTAINER(window), toggle_button);
  gtk_window_set_title(GTK_WINDOW(window), APP_TITLE);
  gtk_window_set_default_size(GTK_WINDOW(window), 400, 400);

  gtk_widget_set_valign(toggle_button, GTK_ALIGN_CENTER);
